...
<span class="TextenoirR">(Vous pourrez modifier ces informations apr&eacute;s votre connexion &agrave; GestAssoPhp)</span>
	...
	<table width="98%" summary="informations générales">
		...
		<tr>
			...
			<td><input type="text" name="email_adht" id="email_adht" title="Exemple localhost " value="{$data_adherent.email_adht}" size="32"  maxlength="50" tabindex="2"/> {if $erreur_saisie.email} <span class="erreur-Jaunerouge">&nbsp;{$erreur_saisie.email}</span>{/if}</td>
		</tr>
		<tr>
			<th class="LignegrisRight">Mot de passe :</th>		
			<td><input type="password" name="pass_adht1" id="pass_adht1" title="Exemple ma_base" value="{$data_adherent.pass_adht1}" size="32"  maxlength="50" tabindex="3"/>{if $erreur_saisie.mdp} <span class="erreur-Jaunerouge">{$erreur_saisie.mdp}</span>{/if}</td>
		</tr>
		<tr>
