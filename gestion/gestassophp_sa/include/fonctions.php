...
/**
 *  Fonction   passe du format 2006-02-12 pour affichage apres requête SQL
...
{
// echo 'DEBUG voir1 = '.$date;
    if ($date) {
// echo 'DEBUG  Date->2 = '.$date;
	// Convertir date MySQL en date FR : 2009-07-14 => 14/07/2009
...
/**
*  Fonction  passe du format 2006-08-16 11:48:22 --> 12/02/2006  11:48:22 (format francais) apres requête SQL
*  modifié par fonction  http://dotclear.placeoweb.com/post/PHP-5.3 le 15/10/2009
*/
...
// echo ' DEBUG '.$regs[2].$regs[1].$regs[3] ;
...
function get_post_variable_numeric($nom_var, $par_defaut)
{
	...
	if (!is_numeric($valeur)) {
		$valeur = $par_defaut; // $valeur = ''; renvoi la valeur par défaut
	}
	return $valeur;
}
...
function NumTel($tel)
{
    ...
    $newtel = preg_replace($regex,
        '\\1 \\2 \\3 \\4 \\5',$tel); // mise en forme remplacement de - par espace
    return $newtel; /* Exemple : 03-81-51-45-78  --> 03 81 xx xx 45 78 */
}
...
{
	if ($date) {
		$age = date('Y') - date('Y', strtotime($date)); // 12/01/17 // correction cause non well formed numeric value
