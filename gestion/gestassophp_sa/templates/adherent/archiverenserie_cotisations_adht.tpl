...
{* Affichage NB cotisations -  - NB pages *}
	<table style="width:100%;">
		<tr>
			...
			{/if}&nbsp;&nbsp;<a href="../admin/remplir_preferences.php" title="{language name=admin_liste_adht_date_f_cotis} {language name = titre_admin_preferences}">{language name = liste_listearchiv_adht_cotis}&nbsp;{$date_3112}</a>{* Avant le "Date fin cotisation" *}
			</td>
			...
			<td class="aff_droite-txt">{language name = tpl_pages}<span class="NumPageGras">
				{section name = pageLoop start = 1 loop = $nb_pages}
					...
					{else}
						<a href="archiverenserie_cotisations_adht.php?numpage_affiche={$smarty.section.pageLoop.index}&amp;affiche_nb_adht={$affiche_nb_adht}&amp;filtre_fiche={$filtre_fiche}&amp;select_datedeb={$filtre_datedeb}&amp;select_datefin={$filtre_datefin}" title="{language name = tpl_go_pages}">{$smarty.section.pageLoop.index}</a>
					{/if}
				{/section}</span>
			</td>
		</tr>
...
{* si empty($cotis_adht) *}
		<tr>
			...
			{if !isset($id_adht) || !isset($nom_prenom)}
				{language name = tpl_liste_vide}&nbsp;{else}{language name = tpl_liste_vide_for}: {$nom_prenom}
			{/if}</td>
		</tr>
{/if}
