...
<form method="post" name="maform" action="remplir_message_adht.php">
    <table style="width:98%;">
		...
		{* Sujet *}
			...
			<td><input type="text" name="email_sujet" id="email_sujet" title="{language name = message_remplir_err_sujet_mail}" value="{if !empty($email_sujet)}{$email_sujet}{/if}" size="65"  maxlength="50" tabindex="3" />
		...
		<td><textarea cols="80" rows="10" name="email_message" id="email_message" title="{language name = message_remplir_err_message_mail}"  tabindex="4">{if !empty($email_message)}{$email_message}{/if}</textarea>
