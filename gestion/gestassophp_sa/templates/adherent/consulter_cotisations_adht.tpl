...
{* Affichage du CONTENU Visualisation cotisation adhérent*}
	<!--div id="titre_aide"><a href='#' style="cursor:pointer;cursor:hand" onclick="javascript :window.open('../aide/a__remplir_cotisation.php','popup','height=300,toolbar=no,location=no,directories=no,status=yes,width=660,resizable=no,scrollbars=yes,top=10,left=10')" title="{language name=title_aide}"><img src='../images/icones/help.gif' alt="Aide" width="20" height="20"/>{language name=aide}</a></div --> 

    <div id="titre">&nbsp;{language name=titre_admin_fiche_visu_cotis_adht} {$affiche_message}</div>
	...
	<table width="100%" summary="cotisation {$adherent_bene}s">
		...
		<tr> 	
			<th class="LignegrisRight" width="25%"> {language name=fiche_cotis_adht_date_enr} :</th>		
